...
- The `JettyServerRule` is used as a **class rule**:
  ...
  - The server will stopped **after all**
  - Iif you want to start/stop server between each test, you can use the rule as a "simple" rule (i.e `@Rule`), but we don't recommend it).
- The default configuration is used.
